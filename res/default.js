	...
	({
		...
		drag: function( event, ui )
		{
			...
			else
			{
				...
				{
					wh += parseInt(tmp.css("left").replace("px",""));
					hi += parseInt(tmp.css("top").replace("px",""));
					tmp = tmp.parent();
				}
			}
			if($("#QIDworkArea").scrollTop() > 0)
			{
				hi += $("#QIDworkArea").scrollTop();
			}
			var 
			guideV, 
			guideH, 
...
function qidactFunc()
{
	this.captionChange = function(val)
	{
		...
		else
		{
			...
			$("#QIDhiddenWorkArea").css("left", "94px");
			$("#QIDhiddenWorkArea").css("top", "76px");
...
$container.on('mousedown', function(e) 
{
	...
	else
	{
		...
		if($('#QIDSELATTRD').val().trim() != "")
		{
			...
			try
			{
				var testvar = $($addedCont).position().left;
			}
			catch(ex)
			{
				$addedCont = $('#QIDworkArea');
			...
			{
				wh += parseInt(curElem.css("left").replace("px",""));
				hi += parseInt(curElem.css("top").replace("px",""));
		...
		$("#QIDhiddenWorkArea").css("left", wh);
		$("#QIDhiddenWorkArea").css("top", hi);
	...
	}).on('mouseup', function(e) 
	{
		...
		// $selection.css("left",($selection.position().left - wh) + "px");
		// $selection.css("top",($selection.position().top - hi) + "px");
